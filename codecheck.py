...
def show_image1():
    ...
    img = Image.open(f"{r}.png")  # Assuming image filenames are "1.png", "2.png", ..., "6.png"
    img = img.resize((200, 200))
    img = ImageTk.PhotoImage(img)
    Lab = tk.Label(root, image=img, height=200, width=200, borderwidth=0)
    Lab.image = img
    Lab.place(x=1000, y=300)
...
# Repeat for other images 2 to 6
...
toss_button.place(x=1200, y=160, height=40)

reset_button = tk.Button(root, text="Reset", font=("Arial", 16), command=reset_game)
reset_button.place(x=1200, y=210, height=40)
